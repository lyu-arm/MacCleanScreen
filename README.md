...
应用包含一个明确标注的作者推广入口，并在每个版本首次启动时显示一次推广提示。MacCleanScreen 不会自动加载推广网站或向其传输数据，只有用户主动选择“访问网站”时才会调用默认浏览器。推广网站由 MacCleanScreen 作者运营，推广业务与本应用的清洁功能相互独立，不代表 OpenAI 官方或授权关系。
